...
const CheckoutPage = () => {
  ...
  // 🔑 FRONTEND Razorpay key (must match backend RAZORPAY_KEY_ID)
  const RAZORPAY_KEY =
    process.env.REACT_APP_RAZORPAY_KEY || "rzp_test_RfXtOK78ACS6AR";

  // 🌐 API base URL (backend)
  const API_URL =
    process.env.REACT_APP_API_URL || "https://hotcolours-final.onrender.com";
  ...
  // Compute totals whenever cart or promo changes
  useEffect(() => {
    const total =
      cart?.items?.reduce(
        (acc, i) => acc + (i.product?.price || 0) * i.quantity,
  ...
  const handlePlaceOrder = async (e) => {
    ...
    try {
      if (formData.paymentMethod === "Card") {
        ...
        // Optional: log why Razorpay fails
  ...
  return (
    <div className="max-w-3xl mx-auto mt-10 p-6 bg-white shadow-xl rounded-2xl border border-gray-200">
      ...
      <div className="border rounded-xl p-5 bg-gray-50 shadow-inner">
        ...
        <hr className="my-3" />
        <div className="flex justify_between text-gray-700">
          <span>Subtotal</span>
          <span>
            ₹{" "}
            {cart?.items?.reduce(
