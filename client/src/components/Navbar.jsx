 ...
 * - Mobile: hamburger => right-side slide-over drawer (accessible)
 */
export default function Navbar() {
  ...
  useEffect(() => {
    if (drawerOpen) {
      ...
      // focus close button after a short delay (ensures element is rendered)
  ...
  useEffect(() => {
    function onKey(e) {
      ...
      // trap focus inside drawer when open (simple)
    ...
    <>
      ...
      >
        <div className="flex items-center gap-3 bg-blue-600 text-white px-4 py-4">
          ...
          <div className="font-semibold">
            {isSignIn ? `Hello, ${user?.name?.split(" ")[0]}` : "Welcome"}
          </div>
          <button
            ref={closeBtnRef}
            onClick={closeDrawer}
        ...
        <nav className="p-4 space-y-2">
          ...
          <button
            ...
            className="w-full text-left px-3 py-2 rounded-md hover:bg-slate-100 flex items-center justify-between"
          >
          ...
          <Link to="/" onClick={closeDrawer} className="block px-3 py-2 rounded-md hover:bg-slate-100">Home</Link>
          <Link to="/contact" onClick={closeDrawer} className="block px-3 py-2 rounded-md hover:bg-slate-100">Contact</Link>
          <Link to="/about" onClick={closeDrawer} className="block px-3 py-2 rounded-md hover:bg-slate-100">About</Link>

          <div className="border-t mt-2 pt-3">
            {isSignIn ? (
              <button onClick={signOut} className="w-full text-left px-3 py-2 rounded-md text-red-600 hover:bg-red-50">
                Sign out
              </button>
            ) : (
              <button onClick={loginPage} className="w-full text-left px-3 py-2 rounded-md text-green-600 hover:bg-green-50">
                Sign in
              </button>
            )}
          </div>
      ...
      <nav className="flex items-center h-[64px] w-full max-w-[1200px] mx-auto justify-between px-4">
        ...
        <div className="hidden md:flex items-center gap-8 text-slate-700 font-semibold">
          ...
          <button onClick={handleCartClick} className="relative flex items-center gap-2">
            <FaShoppingCart />
            <span>Cart</span>
            {cartCount > 0 && <span className="absolute -top-2 -right-6 bg-red-600 text-white text-xs w-5 h-5 flex items-center justify-center rounded-full">{cartCount}</span>}
          </button>
        </div>
